...
public class Robot extends TimedRobot {
  ...
  private DoubleSolenoid solenoidboy;
  //private Compressor compressionboy;
  /**
  ...
  public void robotInit() {
    ...
    solenoidboy = new DoubleSolenoid(2,3);
    //compressionboy = new Compressor(0);
    //compressionboy.setClosedLoopControl(true);

    f_ultrasonic = new Ultrasonic(9,8); // ping, echo
    f_ultrasonic.setAutomaticMode(true);
    f_ultrasonic.setEnabled(true);

    encoder_backRight = new Encoder(dev,0);
...
public void testencoder(){
  ...
  public void teleopPeriodic() {
    ...
    boolean pneumaticwaspressed = xbox.getAButtonPressed();
    boolean isApressed = xbox.getBumperPressed(Hand.kRight);
    boolean pneumaticturnoff = xbox.getBButtonPressed();
    //boolean enabled = compressionboy.enabled();
    int wasitextended = 0;
    if(pneumaticwaspressed == true && wasitextended % 2 == 0) {
      solenoidboy.set(DoubleSolenoid.Value.kForward);
      wasitextended++;
    }
    else if(pneumaticwaspressed == true && wasitextended % 2 == 1) {
      solenoidboy.set(DoubleSolenoid.Value.kReverse);
      wasitextended++;
    }
    else if(pneumaticturnoff == true) {
      solenoidboy.set(DoubleSolenoid.Value.kOff);
      //compressionboy.setClosedLoopControl(false);
    }
    if(isApressed == true && counter % 2 == 1 ) {
      server.setSource(camera1);
      counter ++;
    }
    else if(isApressed == true && counter % 2 == 0) {
      server.setSource(camera0);
      counter ++;
    }
